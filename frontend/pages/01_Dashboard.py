...
def _suspeita_mask(df: pd.DataFrame) -> pd.Series:
    """Coluna booleana `suspeita` (fallback: `is_fraude`)."""
    if df.empty:
        return pd.Series(dtype=bool)
    if "suspeita" in df.columns:
        return _truthy_mask(df["suspeita"])
    if "is_fraude" in df.columns:
        return _truthy_mask(df["is_fraude"])
    return pd.Series(False, index=df.index)


def _df_compras_online(df_tx: pd.DataFrame) -> pd.DataFrame:
    if df_tx.empty or "tipo_pagamento" not in df_tx.columns:
        return pd.DataFrame()
    return df_tx[df_tx["tipo_pagamento"] == "Compra Online"].copy()


def _categoria_compra(banco_destino) -> str:
    """Extrai categoria de `banco_destino` (formato: loja — produto [categoria])."""
    if pd.isna(banco_destino) or not str(banco_destino).strip():
        return "(Sem categoria)"
    s = str(banco_destino).strip()
    if "[" in s and s.endswith("]"):
        cat = s.rsplit("[", 1)[-1].rstrip("]").strip()
        return cat or "(Sem categoria)"
    return s
...
def _find_lavagem_pairs(df_tx: pd.DataFrame) -> pd.DataFrame:
    ...
    tin = df_tx[df_tx["tipo_pagamento"].isin(in_types)][
        ["usuario_id", "cpf", "data_transacao", "valor"]
    ].rename(columns={"data_transacao": "entrada", "valor": "val_in"})
    tout = df_tx[~df_tx["tipo_pagamento"].isin(in_types)][
        ["usuario_id", "data_transacao", "valor"]
    ].rename(columns={"data_transacao": "saida", "valor": "val_out"})
    if tin.empty or tout.empty:
        return pd.DataFrame()
    pairs = tin.merge(tout, on="usuario_id")
    pairs = pairs[
...
def _cashin_sem_historico_agg(df_tx: pd.DataFrame, min_valor: float = 5000) -> pd.DataFrame:
    ...
    if df_tx.empty:
        return pd.DataFrame()
    cand = df_tx[(df_tx["tipo_pagamento"] == "Recebimento") & (df_tx["valor"] >= min_valor)].copy()
    if cand.empty:
        return pd.DataFrame()
    prior = df_tx[["id", "usuario_id", "data_transacao"]].rename(columns={"id": "prior_id", "data_transacao": "dh"})
    m = cand.merge(prior, on="usuario_id")
    has_prior = m[
        (m["dh"] < m["data_transacao"]) & (m["dh"] >= m["data_transacao"] - pd.Timedelta(days=7))
    ]["id"].unique()
...
if not df_tx_raw.empty and not df_usuarios.empty:
    ...
    df_tx = df_tx_raw.merge(
        df_usuarios[u_cols].rename(columns={"id": "usuario_id"}),
        on="usuario_id",
        how="left",
        suffixes=("", "_u"),
    )
...
df_compras_tx = _df_compras_online(df_tx)
...
if not df_tx.empty:
    fig_tipo = px.bar(
        df_tx.groupby("tipo_pagamento")["valor"].sum().reset_index(),
        x="tipo_pagamento",
        y="valor",
        title="Soma de valores por tipo de transação",
        labels={"valor": "Valor (R$)", "tipo_pagamento": "Tipo"},
    )
...
with st.expander("⚡ Explosão de transações por minuto", expanded=True):
    try:
        ...
        if st.button("Gerar gráficos ⚡", key="btn_explosao"):
            sub = _in_date_range(df_tx, "data_transacao", dt_ini, dt_fim)
            if sub.empty:
                st.info("Sem transações no intervalo selecionado.")
            else:
                sub = sub.copy()
                sub["minuto"] = sub["data_transacao"].dt.floor("min")
        ...
        st.warning(f"Erro ao carregar métrica: {e}")
...
with st.expander("🛡️ Alterações de dados sensíveis & Cash-Out", expanded=True):
    try:
        if not df_tx.empty:
            df_co = df_tx[df_tx["tipo_pagamento"].isin(["Cash-Out", "Saque"])][
                ["usuario_id", "data_transacao", "valor", "cpf"]
            ].rename(columns={"data_transacao": "data_hora"}).assign(evento="Cash-Out")
        else:
            df_co = pd.DataFrame()

        if df_co.empty:
            st.info("Nenhum Cash-Out registrado.")
        else:
            st.subheader("📌 Timeline de Cash-Out")
            df_timeline = df_co[["usuario_id", "data_hora", "evento", "cpf"]].copy()
            ...
            fig = px.scatter(
                ...
                color="evento",
                symbol="evento",
                title="⏱️ Timeline de Saques (Cash-Out)",
            ...
            st.dataframe(
                df_co.assign(data_hora=_fmt_dh(df_co["data_hora"])).head(20),
...
with st.expander("💸 Entradas & Saídas em ≤ 5 min (lavagem)", expanded=True):
    try:
        ...
        if df_lav.empty:
            ...
            max_users_count = len(df_top)
            if max_users_count > 1:
                max_users = st.slider(
                    "Exibir top N usuários no gráfico",
                    min_value=1,
                    max_value=max_users_count,
                    value=min(10, max_users_count),
                    step=1,
                    key="lav_top_n",
                )
            else:
                max_users = max_users_count if max_users_count > 0 else 1
...
# ════════════════════════════════════════
# 7) Alterações de senha múltiplas vezes — removido (tabela fatos_usuarios)
# ════════════════════════════════════════
...
with st.expander("💰 Top 5 maiores valores recebidos", expanded=False):
    try:
        ...
        if st.button("Gerar ranking", key="btn_top_receb"):
            ...
            df_in = _since_days(df_tx[df_tx["tipo_pagamento"].isin(filtros_in)], "data_transacao", dias_receb) if not df_tx.empty else pd.DataFrame()
            if df_in.empty:
                st.info("Nenhuma entrada no período selecionado.")
            else:
                top_in = (
                    df_in.groupby(["usuario_id", "cpf", "nome"])["valor"]
                    .sum()
...
with st.expander("🛒 Compras por categoria (on-line)", expanded=False):
    try:
        ...
        if st.button("Gerar gráfico", key="btn_cat"):
            if df_compras_tx.empty:
                st.info("Nenhuma compra on-line encontrada no período selecionado.")
            else:
                sub = _since_days(df_compras_tx, "data_transacao", dias_comp)
                if sub.empty:
                    st.info("Nenhuma compra on-line encontrada no período selecionado.")
                else:
                    sub = sub.copy()
                    sub["categoria"] = sub["banco_destino"].map(_categoria_compra) if "banco_destino" in sub.columns else "(Sem categoria)"
                    df_cat = (
                        sub.groupby("categoria", as_index=False)["valor"]
                        .sum()
                    ...
                    fig_cat = px.bar(
                        ...
                        title=f"Top {len(df_cat)} categorias – compras on-line (últimos {dias_comp} dias)",
                    )
...
with st.expander("💳 Top usuários que mais gastam em compras", expanded=False):
    try:
        ...
        if st.button("Gerar ranking de gastos", key="btn_top_user_comp"):
            df_comb = _since_days(df_compras_tx, "data_transacao", dias_usr)[["usuario_id", "valor"]] if not df_compras_tx.empty else pd.DataFrame()
            if df_comb.empty:
                st.info("Nenhuma compra on-line encontrada no período selecionado.")
            else:
                df_top = (
                    df_comb.groupby("usuario_id")["valor"]
                    .sum()
                    .sort_values(ascending=False)
                    .head(int(top_u))
                    .reset_index()
                ...
                if not df_usuarios.empty:
                    df_top = df_top.merge(
                        df_usuarios[["id", "cpf", "nome"]].rename(columns={"id": "usuario_id"}),
                        on="usuario_id",
                        how="left",
                    )
                elif "cpf" in df_compras_tx.columns:
                    cpf_map = df_compras_tx.groupby("usuario_id")["cpf"].first().reset_index()
                    df_top = df_top.merge(cpf_map, on="usuario_id", how="left")
                fig_topu = px.bar(
                    df_top,
                    x="valor",
                    y="cpf",
                    ...
                    title=f"Top {len(df_top)} usuários por gastos em compras on-line (últimos {dias_usr} dias)",
                )
    ...
    try:
        ...
        if st.button("Calcular média", key="btn_media_cat"):
            if df_compras_tx.empty:
                ...
                sub = _since_days(df_compras_tx, "data_transacao", dias_media)
                if sub.empty:
                    st.info("Nenhuma compra on-line encontrada no período selecionado.")
                else:
                    sub = sub.copy()
                    ...
                    fig_avg = px.bar(
                        ...
                        title=f"Média de compras on-line por categoria (últimos {dias_media} dias)",
                        template="plotly_dark",
                    )
    ...
    try:
        ...
        if st.button("Buscar usuários suspeitos", key="btn_buscar_suspeitos"):
            if df_tx.empty:
                ...
                sus = _since_days(df_tx[df_tx["_suspeita"]], "data_transacao", dias_risk)
                df_suspeitos = (
                    sus.groupby(["usuario_id", "cpf"], as_index=False)
                ...
                if df_suspeitos.empty:
                    ...
                    with st.spinner("Calculando métricas de risco..."):
                        comp_user = df_compras_tx[df_compras_tx["usuario_id"] == user_id] if not df_compras_tx.empty else pd.DataFrame()
                        total_comp = float(
                            _since_days(comp_user, "data_transacao", dias_risk)["valor"].sum()
                        ) if not comp_user.empty else 0.0
                        ...
                        df_radar = pd.DataFrame({
                            "theta": [
                                "Total compras on-line (R$)",
                                "Saldo pendente (R$)",
                                "Transações suspeitas",
                                "Valor suspeitas (R$)",
                            ],
                            "r": [total_comp, saldo_pend, qtd_suspeitas, valor_suspeitas],
                        })
                        ...
                        df_tx_suspeitas = _since_days(
                            df_tx[(df_tx["usuario_id"] == user_id) & df_tx["_suspeita"]],
                            "data_transacao",
                            dias_risk,
                        ...
                        cols = [c for c in ["id", "tipo", "valor", "data_transacao", "motivo", "banco_destino"] if c in show.columns]
    ...
    try:
        ...
        if st.button("Calcular renda e gastos", key="btn_med_renda_all"):
            if df_tx.empty:
                ...
                sub = _since_days(df_tx, "data_transacao", dias_renda)
                tipos_gasto = ("Compra", "Compra Online", "Pagamento", "Transferência", "Saque")
                df_inc = (
                    sub[~sub["tipo_pagamento"].isin(tipos_gasto)]
                    .groupby(["usuario_id", "cpf"], as_index=False)["valor"]
                    .mean()
                    .rename(columns={"valor": "renda_media", "cpf": "CPF"})
                )
                df_out = (
                    sub[sub["tipo_pagamento"].isin(tipos_gasto)]
                    .groupby("usuario_id", as_index=False)["valor"]
                    .sum()
                    .rename(columns={"valor": "gasto_total"})
                )
                df_merge = (
                    df_inc.merge(df_out, on="usuario_id", how="left")
                    .fillna(0)
                ...
                if df_merge.empty:
                    ...
                    max_users_count = len(df_merge)
                    if max_users_count > 1:
                        max_users = st.number_input(
                            "Qtd. de usuários no gráfico",
                            min_value=1,
                            max_value=max_users_count,
                            value=min(15, max_users_count),
                            step=1,
                            key="limite_usuarios",
                        )
                    else:
...
# ════════════════════════════════════════
# 14) Timeline de Fatos do Sistema — removido (tabela fatos_usuarios)
# ════════════════════════════════════════
    ...
    try:
        ...
        if st.button("Gerar gráfico", key="btn_fatos_tx"):
            if df_tx.empty:
                ...
                sub = _in_date_range(df_tx[df_tx["_suspeita"]], "data_transacao", dt_inicio_tx, dt_fim_tx).copy()
                if sub.empty:
                    st.info("Nenhuma transação suspeita no período selecionado.")
                else:
                    sub["dia"] = sub["data_transacao"].dt.date
                    sub["motivo"] = (
                        sub["motivo_suspeita"].fillna("(Sem motivo)")
                        if "motivo_suspeita" in sub.columns
                        else "(Sem motivo)"
                    )
    ...
    try:
        ...
        if st.button("Gerar relatórios de fraudes", key="btn_fraudes"):
            if df_tx.empty:
                ...
                df_fraud = _in_date_range(df_tx[df_tx["_suspeita"]], "data_transacao", fra_ini, fra_fim).sort_values(
                    "data_transacao", ascending=False
                )
                if not df_fraud.empty:
                    df_fraud = df_fraud.assign(
                        data_transacao=_fmt_dh(df_fraud["data_transacao"]),
                        tipo=df_fraud["tipo_pagamento"],
            ...
            cols = [c for c in ["id", "data_transacao", "username", "tipo", "valor", "motivo", "banco_destino"] if c in df_fraud.columns]
...
# ════════════════════════════════════════
# 17–19) Histórico de Edições, Login e Senha — removidos (tabela fatos_usuarios / logs)
# ════════════════════════════════════════
    ...
    try:
        ...
        if st.button("Gerar lista de risco", key="dash_btn_risco"):
            if df_tx.empty:
                ...
                sub = _in_date_range(df_tx[df_tx["_suspeita"]], "data_transacao", risco_de, risco_ate)
                if sub.empty:
                    df_risco = pd.DataFrame()
                else:
                    agg_kw = {
                        ...
                        "ultima_suspeita": ("data_transacao", "max"),
                    }
                    for col in ("username", "email", "banco"):
                        if col in sub.columns:
                            agg_kw[col] = (col, "first")
                    df_risco = (
                        sub.groupby("usuario_id", as_index=False)
                        .agg(**agg_kw)
...
with st.expander("📈 Tendências (Últimos N dias)", expanded=False):
    try:
        ...
        if df_tx.empty or not metricas:
            ...
            df_per = df_tx[df_tx["data_transacao"].dt.date >= cutoff.date()].copy()
            df_per["dia"] = df_per["data_transacao"].dt.date
        ...
        if df_tx.empty:
            ...
            if "estado" not in df_tx.columns and not df_usuarios.empty and "estado" in df_usuarios.columns:
                df_tx_est = df_tx.merge(
                    df_usuarios[["id", "estado"]].rename(columns={"id": "usuario_id"}),
                    on="usuario_id",
                    how="left",
                )
            else:
